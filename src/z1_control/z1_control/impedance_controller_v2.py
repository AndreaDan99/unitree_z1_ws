...
class ImpedanceController(Node):
    ...
    def compute_compensation(self):
        """
        Calcola compensazione dinamica usando Pinocchio + bias costante
        
        URDF sottostima le masse in modo uniforme → aggiungiamo bias fisso
        """
        # Gravità base da Pinocchio
        ...
        # ========================================
        # 🔧 BIAS COSTANTE (URDF sottostima uniformemente)
        # ========================================
        # Differenza misurata: ~7 Nm costanti su tutti gli angoli
        # Aggiungiamo correzione FISSA (non configuration-dependent)
        
        GRAVITY_BIAS_Q2 = 7.5  # Nm - bias costante su joint 2
        tau_comp[1] += GRAVITY_BIAS_Q2
        
        # Opzionale: bias anche su q3 se necessario
        # GRAVITY_BIAS_Q3 = 3.0  # Nm
        # tau_comp[2] += GRAVITY_BIAS_Q3
        
        # 🔍 DEBUG: Log gravità ogni 1 secondo
        ...
        if self.debug_counter % 500 == 0:
            q2_deg = np.rad2deg(self.q[1])
            old_bias = 20.0 * np.cos(self.q[1])
            tau_total_q2 = tau_gravity[1] + GRAVITY_BIAS_Q2
            
            self.get_logger().info('─'*70)
            self.get_logger().info(f'🔧 COMPENSAZIONE GRAVITÀ (q2={q2_deg:.1f}°)')
            self.get_logger().info(f'   Pinocchio q2:    {tau_gravity[1]:6.2f} Nm')
            self.get_logger().info(f'   Bias fisso:      {GRAVITY_BIAS_Q2:6.2f} Nm')
            self.get_logger().info(f'   TOTALE q2:       {tau_total_q2:6.2f} Nm')
            self.get_logger().info(f'   Vecchio (var):   {old_bias:6.2f} Nm')
            self.get_logger().info(f'   Differenza:      {(tau_total_q2-old_bias):6.2f} Nm')
            self.get_logger().info('─'*70)
